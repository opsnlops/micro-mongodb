...
#ifdef BSON_OS_UNIX
/* For sched_yield(). newlib-nano (used by pico-sdk) ships <sched.h> but
 * gates the declaration behind feature-test macros that aren't set here.
 * Declaring it explicitly is safe everywhere -- the prototype is fixed by
 * POSIX. Definition lives in src/bson/port_pico.c for the Pico build. */
#include <sched.h>
