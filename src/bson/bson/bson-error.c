...
// micro-mongodb: arm-none-eabi-gcc + newlib-nano lacks uselocale/strerror_l,
// so for ARM (and any freestanding-ish target) we use plain `strerror`.
#if !defined(_WIN32) && !defined(__APPLE__) && !defined(__arm__) && !defined(__riscv)
                 ...
                 size_t buflen BSON_MAYBE_UNUSED) /* IN */
{
   ...
   // micro-mongodb on arm-none-eabi/RISC-V freestanding: newlib-nano is
   // single-threaded; plain strerror is safe and avoids strerror_l/locale_t
   // which aren't available.
   {
      char *msg = strerror (err_code);
      if (msg) {
         (void) bson_strncpy (buf, msg, buflen);
         ret = buf;
      }
   }
#elif defined(_WIN32)
